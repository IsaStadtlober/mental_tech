...
} from "@/constants/professor/activityForm";
import type {
  Activity,
  ActivityStatus,
  FileType,
...
import type {
  ActivityFormData,
} from "@/types/professor/activityForm";
...
export function useActivityForm({
  ...
  const [rewardName, setRewardName] = useState(
    activity?.reward?.name ?? ACTIVITY_FORM_DEFAULTS.rewardName,
  );
  const [rewardType, setRewardType] = useState<RewardType>(
    activity?.reward?.type ?? ACTIVITY_FORM_DEFAULTS.rewardType,
  );
  ...
  const formIsValid =
    titleIsValid && instructionIsValid && attachmentIsValid && audienceIsValid;

  const studentSelectionLabel = useMemo(
    () => getActivityFormStudentSelectionLabel(selectedStudentNames.length),
  ...
  function handleSave(
    ...
    onSave({
        ...
        attachmentName,
        attachmentType,
        attachmentUri, // propriedade com a URI do arquivo
        rewardName,
        rewardType,
        status,
    });
