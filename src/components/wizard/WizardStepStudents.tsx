...
export function WizardStepStudents({
  onBack,
  onFinish,
}: WizardStepStudentsProps) {
  ...
  const handleBatchImport = async () => {
    Alert.alert(
      WIZARD_CONSTANTS.STEP_STUDENTS.ALERTS.IMPORT_TITLE,
      WIZARD_CONSTANTS.STEP_STUDENTS.ALERTS.IMPORT_MESSAGE,
    );
  };
  ...
  return (
    ...
    >
      <Text style={styles.sectionLabel}>
        {WIZARD_CONSTANTS.STEP_STUDENTS.TEXTS.IMPORT_SECTION}
      </Text>
      ...
      >
        ...
        <Text style={styles.uploadTitle}>
          {WIZARD_CONSTANTS.STEP_STUDENTS.TEXTS.UPLOAD_TITLE}
        </Text>

        <Text style={styles.uploadSubtitle}>
          {WIZARD_CONSTANTS.STEP_STUDENTS.TEXTS.UPLOAD_SUBTITLE}
        </Text>
      </TouchableOpacity>
      ...
      <View style={styles.orRow}>
        ...
        <Text style={styles.orText}>
          {WIZARD_CONSTANTS.STEP_STUDENTS.TEXTS.OR}
        </Text>
        <View style={styles.orLine} />
      </View>
        ...
        {students.map((student, index) => (
          <View key={index} style={styles.studentEntryBlock}>
            <View style={styles.studentBlockHeader}>
              <Text style={styles.studentBlockTitle}>
                {WIZARD_CONSTANTS.STEP_STUDENTS.TEXTS.STUDENT_NUMBER}{" "}
                {index + 1}
              </Text>
              {students.length > 1 && (
                <TouchableOpacity
                ...
                >
                  <Text style={styles.removeStudentText}>
                    {WIZARD_CONSTANTS.STEP_STUDENTS.TEXTS.REMOVE_BUTTON}
                  </Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            <FormField
              ...
              placeholder={
                WIZARD_CONSTANTS.STEP_STUDENTS.PLACEHOLDERS.STUDENT_NAME
              }
              preset="educator"
            />
            <FormField
              label={
                WIZARD_CONSTANTS.STEP_STUDENTS.LABELS.STUDENT_GUARDIAN_EMAIL
              }
              value={student.guardian_email}
              onChangeText={(value) =>
                updateStudent(index, "guardian_email", value)
              }
              placeholder={
                WIZARD_CONSTANTS.STEP_STUDENTS.PLACEHOLDERS
                  .STUDENT_GUARDIAN_EMAIL
              }
              preset="educator"
              keyboardType="email-address"
            />
            {!isEmailValid(student.guardian_email || "") &&
            student.guardian_email.trim().length > 0 ? (
              <Text style={styles.inputErrorText}>E-mail inválido.</Text>
            ) : null}
          </View>
        ))}
      </View>
      ...
      >
        <Text style={styles.addStudentText}>
          {WIZARD_CONSTANTS.STEP_STUDENTS.TEXTS.ADD_MORE}
        </Text>
      </TouchableOpacity>
    </WizardStepLayout>
  );
