...
export function useEducatorActivation() {
  ...
  const activateProfessor = async (schoolId: string) => {
    ...
    try {
      ...
      if (!schoolRecord?.id) {
        const { data: schoolByProfile, error: schoolByProfileError } =
          await supabase
            .from("schools")
            .select("id")
            .eq("profile_id", resolvedSchoolId)
            .maybeSingle();

        if (schoolByProfileError) throw schoolByProfileError;
        if (!schoolByProfile?.id) {
          throw new Error(
            "Não foi possível localizar a escola pelo ID informado.",
          );
        }
